...
function renderMissionsList(missions, checkinState, validReferrals) {
...
function createMissionItemHTML(mission, validReferrals) {
    validReferrals = validReferrals || 0;
    ...
    if (mission.action_url && mission.action_url.indexOf('#referral_') === 0) {
        var target = parseInt(mission.action_url.split('_')[1]);
        if (!isNaN(target)) {
    ...
    var actionAttr = mission.is_completed ? '' : 'onclick="doMission(\'' + mission.id + '\', \'' + (mission.action_url || '') + '\')"';

    return '<div class="mission-item ' + hotClass + '"><div class="mission-info"><div class="mission-title">' + hotIcon + title + '</div><div class="mission-reward">+' + mission.reward_xu + ' xu</div></div><div class="mission-action"><button class="btn-mission ' + btnClass + '" ' + actionAttr + '>' + btnText + '</button></div></div>';
}

async function doCheckinMission() {
    var btn = document.getElementById('btnMissionCheckin');
    btn.disabled = true;
...
var currentDonateMissionId = null;

function doMission(id, url) {
    if (url) {
        if (url === '#enter_referral') {
            var code = prompt("Nhập mã giới thiệu (6 ký tự):");
            if (code) submitReferralCode(code);
            return;
        } else if (url.indexOf('#referral_') === 0) {
            var target = parseInt(url.split('_')[1]);
...
async function confirmDonate() {
    ...
    var btn = document.getElementById('btnConfirmDonate');
    var oldText = btn.innerText;
    btn.innerText = "Đang gửi yêu cầu...";
    btn.disabled = true;
    try {
        var token = localStorage.getItem('sa_token');
        var res = await fetch('/api/auth?action=request_deposit', {
            method: 'POST',
...
async function submitReferralCode(code) {
    ...
    if (!token) return;
    try {
        var res = await fetch('/api/auth?action=enter_referral', {
            method: 'POST',
...
async function claimReferralMission(missionId, target) {
    ...
    if (!token) return;
    try {
        var res = await fetch('/api/auth?action=claim_referral', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
...
function copyReferralCode() {
    var code = document.getElementById('myReferralCode').innerText;
    if (code && code !== '---') {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(code).then(function() {
                alert('Đã copy mã giới thiệu: ' + code);
            }).catch(function() {
                fallbackCopy(code);
            });
        } else {
