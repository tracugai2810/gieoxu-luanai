...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // Trừ xu trước khi gọi API (Nếu lỗi sẽ hoàn lại sau)
...
(KHÔNG được thuật lại thông tin quẻ vì người xem đã thấy rồi. Thay vào đó, hãy ĐỐI CHỨNG: Liên hệ thông tin quẻ với câu hỏi và hoàn cảnh thực tế của người hỏi. Ví dụ: hào Thế đại diện cho ai, hào Ứng đại diện cho ai/điều gì trong câu hỏi. Các lục thân trong quẻ tương ứng với người/sự việc nào trong thực tế. Đánh giá xem quẻ phản ánh tình huống thực tế có đúng không, có phù hợp với câu hỏi không.)
...
[QUY TẮC TRÌNH BÀY]:
- Mỗi luận điểm riêng biệt PHẢI xuống dòng mới, dùng dấu gạch đầu dòng "- ".
- KHÔNG viết nhiều luận điểm dồn trong 1 đoạn dài. Phải tách rõ ràng.
- Dùng **in đậm** cho các thuật ngữ quan trọng.`;
        ...
        await supabaseRequest('/rest/v1/xu_transactions', 'POST', {
            ...
            amount: -cost,
            type: 'ai_usage',
            description: `Luận giải quẻ tự động (Gói ${modelConfig ? modelConfig.name : 'Cơ Bản'})`
        });
